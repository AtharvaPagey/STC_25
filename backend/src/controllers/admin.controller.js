...
const modelfinetuner = asyncHandler(async (req, res) => {
  ...
  if (!newData || !Array.isArray(newData) || newData.length === 0) {
    throw new ApiError(
      400,
      "Fine-tuning data is required and must be a non-empty array."
    );
  }
  ...
  try {
    ...
    console.error(
      "Error communicating with the fine-tuning service:",
      error.message
    );
    throw new ApiError(
      500,
      "The fine-tuning service is currently unavailable or failed to start the job."
    );
  }
});
...
const getAllUsers = asyncHandler(async (req, res) => {
  const users = await User.find({}).select(
    "-password -refreshToken -prevDisease -firebaseUID"
  );
...
const checkAdminStatus = asyncHandler(async (req, res) => {
  // req.user is already verified to be an admin by the verifyAdmin middleware
  return res
    .status(200)
    .json(new ApiResponse(200, { isAdmin: true }, "Admin status verified"));
});
